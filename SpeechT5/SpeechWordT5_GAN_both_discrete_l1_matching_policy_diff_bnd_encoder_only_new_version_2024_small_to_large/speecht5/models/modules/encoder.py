...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, tgt_dict=None, embed_tokens=None, src_dict=None, src_embed_tokens=None):
        self.args = args
        super().__init__(None)
        ...
        if src_dict is not None:
            self.src_proj = Linear(args.encoder_embed_dim, len(src_dict))
        else:
            self.src_proj = None
    ...
    def forward(
        ...
        if self.src_proj:
            src_for_ctc = self.src_proj(self.dropout_module(encoder_out["encoder_out"][0]))
        else:
            src_for_ctc = None

        encoder_out["encoder_out_for_ctc"] = [x_for_ctc] # T x B x C
        encoder_out["src_encoder_out_for_ctc"] = [src_for_ctc] # T x B x C
