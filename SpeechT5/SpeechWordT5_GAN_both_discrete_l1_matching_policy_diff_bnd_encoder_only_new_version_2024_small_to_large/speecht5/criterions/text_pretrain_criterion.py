...
class TextPretrainCriterion(FairseqCriterion):
    def __init__(self, task, sentence_avg, bart_weight, masked_lm_weight, unigram_weight, loss_weights=None):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        net_output, codebook_out, encoder_output, masked_lm_results, unigram_loss = model(**sample["net_input"])
        ...
        loss_m_list = []
        logp_m_list = model.get_logits(masked_lm_results, True)
        targ_m_list = model.get_targets(None, masked_lm_results, True)
        assert len(logp_m_list) > 0
        for i, (logp_m, targ_m) in enumerate(zip(logp_m_list, targ_m_list)):
            loss_m = F.cross_entropy(logp_m, targ_m, reduction="sum" if reduce else "none")
            loss_m_list.append(loss_m)
        
        masked_lm_loss = sum(loss_m_list)
        loss = self.bart_weight * bart_loss
        loss += self.masked_lm_weight * masked_lm_loss
        loss += self.unigram_weight * unigram_loss
        logging_output = {
            "loss": loss.item(),
            "ntokens": sample["ntokens"],
            "nsentences": sample["target"].size(0),
            "bart_loss": bart_loss.item(),
            "masked_lm_loss": self.masked_lm_weight * masked_lm_loss.item(), 
            "unigram_loss": self.unigram_weight * unigram_loss.item(), 
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        unigram_loss_sum = sum(log.get("unigram_loss", 0) for log in logging_outputs)
