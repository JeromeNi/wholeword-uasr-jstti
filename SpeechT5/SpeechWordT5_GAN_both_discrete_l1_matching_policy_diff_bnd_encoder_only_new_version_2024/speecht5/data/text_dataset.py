...
def collate(
    ...
    if samples[0].get("unmasked_source", None) is not None:
        unmasked_src_tokens = merge(
            "unmasked_source",
            left_pad=left_pad_source,
            pad_to_length=pad_to_length["source"] if pad_to_length is not None else None,
        )
        unmasked_src_tokens = unmasked_src_tokens.index_select(0, sort_order)
...
class TextPretrainDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            if self.permute_sentence_ratio > 0.0:
                print('Warning!','permute_sentence_ratio > 0.0')
            ...
            if self.insert_ratio > 0:
                print('Warning!','insert_ratio > 0.0')
                source = self.add_insertion_noise(source, self.insert_ratio)

            if self.rotate_ratio > 0.0 and np.random.random() < self.rotate_ratio:
                print('Warning!','rotate_ratio > 0.0')
        ...
        return {
            "id": index,
            "source": source,
            "unmasked_source": unmasked_source,
            "target": target,
        }
    ...
    def add_insertion_noise(self, tokens, unmasked_tokens, p):
        ...
        unmasked_result = torch.LongTensor(n + len(tokens)).fill_(-1)
    ...
    def collater(self, samples, pad_to_length=None):
        ...
        """
        return collate(
            samples, self.vocab.pad(), self.eos, self.vocab, task_name = self.task_name, pad_to_length=pad_to_length
        )
