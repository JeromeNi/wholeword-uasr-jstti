...
class UnpairedAudioTextConfig(FairseqDataclass):
    ...
    sample_without_replacement: bool = field(default=True, metadata={"help": "use sample without replacement for sampling unpaired data"})
...
class UnpairedAudioText(FairseqTask):
    ...
    def optimizer_step(self, optimizer, model, update_num):
        ...
        # else:
        #     optimizer.step()
        #print(model.generator.proj.weight)
        #print(model.generator.proj.weight.grad)
        weight_grad = torch.sum(torch.abs(model.generator.proj.weight.grad))
        #print(weight_grad)
        # print('weights BEFORE opt step:')
        # print(model.generator.proj.weight)
        # print(torch.sum(torch.abs(model.generator.proj.weight)))
        if weight_grad.item() > 0.0:
            optimizer.step()
        # print('weights AFTER opt step:')
        # print(model.generator.proj.weight)
        # print(torch.sum(torch.abs(model.generator.proj.weight)))
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if has_unpaired_text:
            ...
            self.datasets[split] = RandomInputDataset(
                ...
                sample_without_replacement=self.cfg.sample_without_replacement,
            )
