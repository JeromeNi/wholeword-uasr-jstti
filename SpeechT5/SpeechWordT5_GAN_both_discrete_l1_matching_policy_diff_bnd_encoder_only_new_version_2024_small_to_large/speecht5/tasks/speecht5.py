...
from speecht5.data.random_input_dataset import RandomInputDataset
...
TASK_NAME = ["s2t", "t2s", "s2s", "s2c", "pretrain", "text_pretrain_only", "discrete_pretrain", "discrete_pretrain_gan"]
...
class SpeechT5Task(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--use-valid-s2t",
            action="store_true",
            help="use s2t-like criterion for validation",
        )
    ...
    def __init__(self, args, dicts, config):
        ...
        if self.t5_task in ['s2t', 't2s', 's2s']:
            self.max_pos = [self.args.max_speech_positions]
        elif self.t5_task == 'pretrain' or self.t5_task == 'text_pretrain_only' or self.t5_task == 'discrete_pretrain' or self.t5_task == 'discrete_pretrain_gan':
            self.max_pos = [self.args.max_speech_positions, self.args.max_text_positions]

        self.mask_idx = self.dicts["text"].add_symbol("<mask>")
        self.src_mask_idx = self.dicts["audio"].add_symbol("<mask>")
        ...
        self.seed = args.seed
        self.use_valid_s2t = args.use_valid_s2t
    ...
    def setup_task(cls, args, **kwargs):
        ...
        if args.t5_task == 'text_pretrain_only' and not hasattr(args, "shuffle_instance"):
            args.shuffle_instance = False
        if args.t5_task == 'discrete_pretrain' and not hasattr(args, "shuffle_instance"):
            args.shuffle_instance = True
        if args.t5_task == 'discrete_pretrain_gan' and not hasattr(args, "shuffle_instance"):
            args.shuffle_instance = False
        ...
        if args.t5_task == "discrete_pretrain" or args.t5_task == "discrete_pretrain_gan" or args.t5_task == "s2t":
            dicts["audio"] = Dictionary.load(op.join(args.data, "dict.audio0.txt"))
        if args.t5_task == "pretrain" or args.t5_task == "text_pretrain_only":
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if self.t5_task == "s2t":
            ...
            manifest = f"{self.args.data}/{speech_split}/{speech_split}"
            procs = [LabelEncoder(self.dicts["text"])]
            src_procs =[LabelEncoder(self.dicts["audio"])]
            src_bpe_tokenizer = self.build_bpe(self.args)
            self.datasets[split] = SpeechToTextDataset(
                manifest_path=manifest,
                sample_rate=self.args.sample_rate,
                label_processors=procs,
                max_keep_sample_size=None,
                min_keep_sample_size=2,
                max_sample_size=self.args.max_speech_sample_size,
                normalize=self.args.normalize,
                store_labels=True,
                tgt_dict=self.dicts["text"],
                tokenizer=bpe_tokenizer,
                src_dict=self.dicts["audio"],
                src_tokenizer=src_bpe_tokenizer,
                src_label_processors=src_procs,
            )
        ...
        elif self.t5_task == "text_pretrain_only":
            is_train_split = ("train" in split)
            pretrain_datasets = []
            _, text_split = split.split('|')
            
            ## Text pre-train
            split_path = op.join(self.args.data, text_split, text_split)
            ...
            logger.info(
                "Task: {0}, Loaded {1} samples of denoising_dataset".format(
                    'bart',
                    len(pretrain_datasets[0]),
                )
            )
            
            self.datasets[split] = pretrain_datasets[0]
            ...
            ## Speech pre-train
            manifest = f"{self.args.data}/{speech_split}/{speech_split}"
            spkemb_path = f"{self.args.data}/spkemb"
            dicts = self.dicts["hubert"]
            pad_list = [dict.pad() for dict in dicts]
            eos_list = [dict.eos() for dict in dicts]
            procs = [LabelEncoder(dict) for dict in dicts]
            paths = [
                f"{self.args.hubert_label_dir}/{speech_split}/{speech_split}.{l}" for l in self.args.hubert_labels
            ]
            ...
            pretrain_datasets.append(
                SpeechPretrainDataset(
                    ...
                    mask = self.args.mask, 
                    mask_random = self.args.mask_random, 
                    iid_noise_target=self.args.iid_noise_target,
                    replace_length=self.args.replace_length, 
                    uni_mask_idxs=None,
                )
            )
        ...
        elif self.t5_task == "discrete_pretrain_gan":
            is_train_split = ("train" in split)
            pretrain_datasets = []
            speech_split, text_split = split.split('|')
            ...
            speech_mask_whole_words = (
                get_whole_word_mask(self.args, self.dicts["audio"])
                if self.args.mask_length != "subword"
                else None
            )

            ## speech units pre-train
            split_path = op.join(self.args.data, speech_split)
            ## Speech pre-train
            manifest = f"{self.args.data}/{speech_split}/"
            # hubert v1: pad_audio=True, random_crop=False;
            self.args.dec_weight = getattr(self.args, "dec_weight", 1.0)
            pretrain_datasets.append(
                SpeechPretrainDataset(
                    ...
                    mask_whole_word=speech_mask_whole_words,
                    mask_length = self.args.mask_length, 
                    poisson_lambda = self.args.poisson_lambda,
                    mask = self.args.mask, 
                    mask_random = self.args.mask_random, 
        ...
        elif self.t5_task == "discrete_pretrain":
            print('discrete_pretrain', split)
            is_train_split = ("train" in split)
            pretrain_datasets = []
            speech_split, text_split = split.split('|')

            ## speech units pre-train
            split_path = op.join(self.args.data, speech_split, speech_split)
            speech_bart_dataset = data_utils.load_indexed_dataset(
                split_path,
                self.dicts["audio"],
                self.args.dataset_impl,
                combine=combine,
            )
            if speech_bart_dataset is None:
                raise FileNotFoundError(
                    "Dataset not found: {} ({})".format(speech_split, split_path)
                )
            speech_bart_dataset = StripTokenDataset(speech_bart_dataset, self.dicts["audio"].eos())
            speech_bart_dataset = maybe_shorten_dataset(
                speech_bart_dataset,
                speech_split,
                self.args.shorten_data_split_list,
                self.args.shorten_method,
            ...
            # create continuous blocks of tokens
            speech_bart_dataset = TokenBlockDataset(
                speech_bart_dataset,
                speech_bart_dataset.sizes,
                self.args.tokens_per_sample - 2,  # one less for <s> and one for </s>
                pad=self.dicts["audio"].pad(),
                eos=self.dicts["audio"].eos(),
                break_mode=self.args.sample_break_mode,
                document_sep_len=0,
            )
            # prepend beginning-of-sentence token (<s>, equiv. to [CLS] in BERT)
            speech_bart_dataset = PrependTokenDataset(speech_bart_dataset, self.dicts["audio"].bos())
            speech_bart_dataset = AppendTokenDataset(speech_bart_dataset, self.dicts["audio"].eos())
            mask_whole_words = (
                get_whole_word_mask(self.args, self.dicts["audio"])
                if self.args.mask_length != "subword"
                else None
    ...
    def optimizer_step(self, optimizer, model, update_num):
        ...
        # else:
        # print(model.speech_encoder_prenet.policy_network_logits.grad)
    ...
    def build_model(self, args):
        try:
            ...
            args.input_feat_per_channel = 768
    ...
    def build_bpe(self, args):
        if self.config is not None:
            ...
            if self.args.bpe_tokenizer is None:
                logger.info("tokenizer: space")
                return encoders.build_tokenizer(Namespace(**{"tokenizer": "space"}))
            else:
