...
class Wav2vecU_Word(BaseFairseqModel):
    ...
    def __init__(self, cfg: Wav2vecU_WordConfig, target_dict):
        ...
        self.max_text_positions = 100
        if not self.position_skipgram:
    ...
    def forward(
        ...
        for skip in range(1, self.skip_size+1):
            if self.position_skipgram:
                ...
                if self.training and self.update_num % 2 == 0:
                    self.saved_dense_skipgram.data[skip - 1, :, :] = self.saved_dense_skipgram.data[skip - 1, :, :] + count_dense_x
            skip_dense_x.append(count_dense_x)
        if self.skip_size > 0:
            skip_dense_x_for_loss = torch.stack(skip_dense_x, dim = 0)
        ...
        for skip in range(1, self.skip_size+1): 
            if self.position_skipgram:
                ...
                if self.training and self.update_num % 2 == 0:
                    self.saved_token_skipgram.data[skip - 1, :, :] = self.saved_token_skipgram.data[skip - 1, :, :] + count_token_x
        ...
        if d_step:
            ...
            if self.gan_type == "L1":
                count_dense_uni = F.pad(dense_x.sum(0), (0, 0, 0, self.max_text_positions - dense_x.size(1)), mode='constant', value=0)
                count_token_uni = F.pad(token_x.sum(0), (0, 0, 0, self.max_text_positions - token_x.size(1)), mode='constant', value=0)
                if self.training and self.update_num % 2 == 0:
                    # print("unigrams updated")
                ...
                if self.update_num % 2 == 1:
                    loss_dense = F.l1_loss(self.saved_dense_unigram - count_dense_uni.detach() + count_dense_uni, self.saved_token_unigram, reduction="sum")
                    # print('unigram loss', loss_dense)
                else:
                    loss_dense = (F.l1_loss(self.saved_dense_unigram - count_dense_uni.detach() + count_dense_uni, self.saved_token_unigram, reduction="sum") - F.l1_loss(self.saved_dense_unigram - count_dense_uni.detach() + count_dense_uni, self.saved_token_unigram, reduction="sum"))
                if self.skip_size > 0:
                    if self.update_num % 2 == 1:
                        loss_dense +=  F.l1_loss(self.saved_dense_skipgram- skip_dense_x_for_loss.detach() + skip_dense_x_for_loss, self.saved_token_skipgram, reduction="sum")
                        # print('skipgram loss', F.l1_loss(self.saved_dense_skipgram- skip_dense_x_for_loss.detach() + skip_dense_x_for_loss, self.saved_token_skipgram, reduction="sum"))
